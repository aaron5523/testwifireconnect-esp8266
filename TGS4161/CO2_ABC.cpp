...
void CO2_ABC::readSensorData(RunningAverage raTempC) {
	double dd = _tgs.getVolts(_tgs.analogReadFine(ANALOG_READ_PRECISION));
	raCO2mv.addValue(_tgs.getCO2_Mv(dd, true, raTempC));
	raCO2mvNoTempCorr.addValue(_tgs.getCO2_Mv(dd, false, raTempC));
}

void CO2_ABC::processCO2SensorData() {
	...
	double co2MvAdj = raCO2mv.getAverage()
			+ _tgs.ppm2mv((double) cfg_lowest_co2_ppm);
...
void CO2_ABC::computeCO2PPM() {
	sPPM = _tgs.mv2ppm(getCO2MaxMv() - raCO2mv.getAverage());
}

void CO2_ABC::storeCurrentCO2MaxMv() {
	EEPROM.put(EE_FLT_CURRENT_PERIOD_CO2_HIGHESTMV, currentCO2MaxMv);
}

void CO2_ABC::co2OnOneHour() {
	storeCurrentCO2MaxMv();
	if (eeAddHourAndReturn() == cfg_abc_resetHours) {
		for (byte i = 0; i < 4; i++)
			EEPROM.put(EE_4B_HOUR + i, (byte) 0);
...
byte CO2_ABC::eeAddHourAndReturn() {
	byte maxH = 0, pos = 0, r = 0, i = 0;
	for (; i < 4; i++) {
		if (EEPROM.get(i + EE_4B_HOUR, r) && r > maxH && r != 255) {
			maxH = r;
...
void CO2_ABC::debugInfoCO2ABC() {
	Serial << F("cmv:") << raCO2mv.getAverage() << F("\n");
	Serial << F("cNT:") << raCO2mvNoTempCorr.getAverage() << F("\n");
	Serial
			<< F(
					"dif:") << raCO2mvNoTempCorr.getAverage() - raCO2mv.getAverage() << F("\n");
	Serial << F("cMv:") << currentCO2MaxMv << F("\n");
	Serial << F("pMv:") << prevCO2MaxMv << F("\n");
//  Serial << F("tem:") << raTempC.getAverage() << endl;
	// Serial << F("upt:") << millis2min() << endl;
	Serial << F("hou:") << eeGetHours() << F("\n");
	Serial << F("sPP:") << sPPM << F("\n");
}

boolean CO2_ABC::timePassed(unsigned long since, unsigned long interval) {
	return (unsigned long) (millis() - since) > interval;
}

void CO2_ABC::clearEEPROM() {
	for (int i = 0; i < 300; i++)
		EEPROM.write(i, 255);
}

int CO2_ABC::getFloat(float f) {
	return (f - (int) f) * 10;
}
